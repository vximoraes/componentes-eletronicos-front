        ...
        <div className="px-6 pb-6 space-y-6">
          ...
          {entradaMutation.error && (
            <div className="p-3 bg-red-100 border border-red-400 text-red-700 rounded text-sm">
              <div className="font-medium">Erro ao registrar entrada:</div>
              <div className="mt-1">
                {(entradaMutation.error as any)?.response?.data?.message || 
                  (entradaMutation.error as any)?.message || 
                  'Erro desconhecido'}
              </div>
              {(entradaMutation.error as any)?.response?.data?.errors && (
                <div className="mt-2">
                  <div className="font-medium">Detalhes:</div>
                  <ul className="list-disc list-inside mt-1">
                    {(entradaMutation.error as any).response.data.errors.map((err: any, index: number) => (
